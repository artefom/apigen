...
use actix_web::{
    get, http::StatusCode, middleware::NormalizePath, web, App, HttpResponse, HttpServer,
    ResponseError,
};
...
impl StatusCoded for HelloUserError {
    fn status_code(&self) -> StatusCode {
        match self {
            HelloUserError::InvalidCharacters => StatusCode::BAD_REQUEST,
        }
    }
}
    ...
    S: Send + Sync + 'static,
{
    ...
    HttpServer::new(move || {
        App::new()
            ...
            .service(docs)
            .route("/hello/{user}", web::get().to(T::hello_user))
    })
    .bind(bind)?
    .run()
    .await?;
